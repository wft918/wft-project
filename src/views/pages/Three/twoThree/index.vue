...
export default {
  ...
  methods: {
    ...
    loadObjModel() {
      ...
      loader.load(`${this.publicPath}model/scene.obj`, (obj) => {
        console.log(obj, 'obj----->>>')
        obj.scale.set(100,100,100)  //  设置模型大小缩放
        obj.position.set(0,0,0)
        // //绕axis轴逆旋转π/16
        // obj.scene.rotateOnAxis(axis,Math.PI/-20);
        // obj.scene.rotateOnAxis(axis,Math.PI/50);
        // obj.rotateY(Math.PI / 2);
        // this.groupBox.add(obj);
        this.scene.add(obj)
      }, (xhr) => {
    ...
    loadGlbModel() {
      ...
      loader.load(`${this.publicPath}model/scene.gltf`, (gltf) => {
        console.log(gltf, 'gltf----->>>')
        gltf.scene.scale.set(100,100,100)  //  设置模型大小缩放
        gltf.scene.position.set(0,0,255)
        let axis = new THREE.Vector3(0,1,0);//向量axis
        gltf.scene.rotateOnAxis(axis,Math.PI/2);
        //绕axis轴逆旋转π/16
        gltf.scene.rotateOnAxis(axis,Math.PI/-20);
